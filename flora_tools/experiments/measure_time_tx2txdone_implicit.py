...
class MeasureTimeTx2TxDoneImplicit(Experiment):
    def __init__(self):
        description = "Measures the time needed from a executed setTx() command (NSS going high) " \
                      "to the Tx Done IRQ on the same node (DIO1 rising edge). The payload is a priori fixed in size."
    ...
    def run(self, bench, iterations=10000):
        ...
        for i in range(0, self.iterations):
            ...
            configuration = RadioConfiguration.get_random_configuration(crc=True, implicit=text_len)
