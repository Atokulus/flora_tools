        ...
        for i in range(0, self.iterations):
            ...
            if wave is not None:
                ...
                item = [dt.datetime.now(), window, self.bench.scope.sample_period, configuration.modulation,
                        configuration.band, configuration.preamble, offset_b, offset_c, valid]
            else:
                item = [dt.datetime.now(), window, self.bench.scope.sample_period, configuration.modulation,
                        configuration.band, configuration.preamble, offset_b, offset_c, np.nan]
    ...
    def analyze(self, df: pd.DataFrame):
        ...
        for mod in mods:
            ...
            def calc_relative_offset(row):
                ...
                math = RadioMath(config)
                return ((pd.Series(
                    [row.offset_1, row.offset_2]) / 8E6 - math.get_preamble_time())) / math.get_preamble_time()
