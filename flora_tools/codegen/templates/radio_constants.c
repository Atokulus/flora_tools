...
const radio_config_t radio_modulations[] =
{
        {% for config in configurations %}
		    { .modem = {{config.modem}}, .bandwidth = {{config.bandwidth}}, .datarate = {{config.datarate}}, .coderate = {{config.coderate}}, .preambleLen = {{config.preamble_len}} },
		{% endfor %}
};
...
{
		{% for band in bands %}
		    { .centerFrequency = {{band.center_frequency}}, .bandwidth = {{band.bandwidth}}, .dutyCycle = {{band.duty_cycle}}, .maxPower = {{band.max_power}} },
		{% endfor %}
